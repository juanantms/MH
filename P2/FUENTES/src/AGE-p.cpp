...
void OperadorCruce(vector<vector<bool>> &padres, int n, int m, double **M) {
    ...
    Cruce(padres[0], padres[1], hijo1, hijo2, n);
...
void AGEPosicion(vector<vector<bool>> &poblacion, int n, int m, double **M) {
...
int main()  {
    ...
    cout << "AGE-posicion\t" << dispersion << endl;
    cout << "Tiempo_AGE-p\t" << tiempo.count() << endl;
